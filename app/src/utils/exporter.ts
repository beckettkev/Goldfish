// file saver (HTML 5)
import * as ContactFileSaver from '../data/filesaver';

export default {
  convertArrayObjectsToCsv: function convertArrayObjectsToCsv(items: Array<any>): string {
    let str: string = '';
    let line: string = '';
    let heading: string = '';
    let index: string = '';

    items.forEach((item: any, i: number): void => {
      line = '';

      for (index in item.Cells) {
        if (i === 1) {
          heading += index + ',';
        }
  ...
  exportCsvStringToFile: function exportCsvStringToFile(csvContent: any, filename: string): void {
  ...
  getContactForExport: function getContactForExport(person: any): any {
    const name: string = person.PreferredName.split(' ');
    const dateRightNow: Date = new Date();

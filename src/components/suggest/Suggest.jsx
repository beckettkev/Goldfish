...
class Suggest extends React.Component {
  ...
  _addTag (tag) {
    ...
    let isUnique = (this.props.tags.filter(function(item, i) {
        return item.name !== tag.name
    })).length === 0;

    let limit = this._maxTags(this.props.tags.length);

    if (typeof tag.name !== 'undefined' && limit && (isUnique || !onlyUnique)) {
        let tags = this.props.tags.concat([tag]);
        this.props.onTagsChange(tags);
        this._clearInput();
    }
  }
  ...
  onSearch(event) {
    ...
    let search = this.props.tags.map(function(tag, i) { 
        return tag.search;
    });

    //take the array values and join them for the search (invoke search in parent)
    this.props.onChange(search.join(' '));
  }
      ...
      if (this.props.tags.length > 0) {
          return(
            <div  styleName='tag-holder'>
                <Button icon='search' label='Search' raised primary onClick={this.onSearch.bind(this)} />
            </div>
          );
      }      
    }
    ...
    if (event.keyCode === 9 || event.keyCode === 13) {
        ...
        if (Object.keys(this.state.tag).length > 0) {  
          ...
          if (this.state.value !== '') {
            ...
            let seperator = this.state.value.indexOf(':') > -1 ? ':' : (this.state.value.indexOf('=') > -1 ? '=' : '');
    ...
    if (this.state !== null) {
      ...
      return (
          <div key='autosuggest-region'className={'animated flipInX'}>
                  ...
                  <Autosuggest key='goldfish-autosuggest'
                      multiSection={true}
...
module.exports = cssModules(Suggest, styles, { allowMultiple: true });
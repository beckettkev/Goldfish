...
const Goldfish:Goldfish = {
    ...
    document.addEventListener('keydown', function launchByKey(e:any):void {
      ...
      if (window.keyWatcher === null && e.keyCode === 18) {
        ...
        if (component === null) {
          ...
          holder.className = 'ms-slideRightIn10';
  ...
  */
  Swim: function Swim():boolean {
    ...
    Goldfish.LoadStyleSheet('https://static2.sharepointonline.com/files/fabric/office-ui-fabric-core/4.1.0/css/fabric.min.css');
    //Goldfish.LoadStyleSheet('https://appsforoffice.microsoft.com/fabric/4.1.0/fabric.components.min.css');
  ...
  Snappy: function Snappy():void {
    if (typeof Goldfish.options.snappy !== 'undefined') {
      if (Goldfish.options.snappy) {
        let clickers:Array<string> = ['move-dragging'];
  ...
  OverrideThemeColours: function OverrideThemeColours():void {
    if (typeof window.jQuery !== 'undefined') {
      if (document.getElementById('component') !== null) {
        ...
        const css:string = '#O365fpcontainerid { z-index:2010; } #component-ghostpane { background-color: ' + window.Goldfish.GetPrimaryColour() + '} #outer-space { position: absolute; z-index: 2000; top: 85px; display: block; right: 0; height: 100%; min-height: 400px; width: 570px; overflow-y: auto; } #outer-space::-webkit-scrollbar { background: #eeeeee } #outer-space::-webkit-scrollbar-thumb { background: #cccccc } #outer-space div.sortable-container div.sortable-item div:hover, #outer-space div.sortable-container div.sortable-item:hover { background-color: ' + colour + '; } #outer-space .gf-component { border-top-color:' + colour + ' !important;} #outer-space .highlight, #component-tabs .active { color:' + colour + ' !important; } #outer-space span.commandor input[type=button], #outer-space input[type=text]:focus, #outer-space input[type=text]:hover { border-color: ' + colour + ' !important; } #outer-space button, #outer-space input[type=button] { background-color: ' + colour + '; color:#ffffff; box-shadow: -1px 4px 8px 0px #ccc; } #outer-space div.nocolour button, #outer-space div.nocolour input[type=button] { background-color: #f4f4f4 !important; } #outer-space div.remove button, #outer-space div.remove input[type=button] { background-color: #666666 !important; } div.switches-with-broomsticks label span[role=switch] { background-color: ' + colour + ' !important; } div.switches-with-broomsticks label span span[role=thumb] { background-color: ' + colour + ' !important; } #outer-space div.Select-option.is-focused { background-color: ' + colour + ' !important; color:#ffffff; }';
...
window.Goldfish.options = {
  title: 'Goldfish',
  css: {
    primary: document.getElementById('O365_NavHeader') ? window.Goldfish.GetPrimaryColour() : '#0066ff'
  },
  properties: '',
  snappy: true
};
